#ifndef PYBINDSTAN_SOCKET_WRITER_HPP
#define PYBINDSTAN_SOCKET_WRITER_HPP
...
/**
 * NOTE: pybindstan makes use of `message_prefix` in an unexpected way!
 *
 * pybindstan uses `message_prefix` to record what messages the socket_writer instance is receiving.
 * In a call to `hmc_nuts_diag_e_adapt`, three socket_writers are used:
 * 1. init_writer
 * 2. sample_writer
 * 3. diagnostic_writer
 *
 * pybindstan uses `message_prefix` to allow the socket_writer to know in what
...
#endif // PYBINDSTAN_SOCKET_WRITER_HPP
