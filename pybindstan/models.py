...
import pybindstan.build_ext
import pybindstan.cache
import pybindstan.compile
...
def calculate_model_name(program_code: str) -> str:
    ...
    - UTF-8 encoded string recording the pybindstan. version
    ...
    # system identifiers
    hash.update(pybindstan.__version__.encode())
...
def import_services_extension_module(model_name: str) -> ModuleType:
    ...
    """
    model_directory = pybindstan.cache.model_directory(model_name)
...
def build_services_extension_module(model_name: str, program_code: str, extra_compile_args: Optional[List[str]] = None) -> str:
    ...
    """
    model_directory_path = pybindstan.cache.model_directory(model_name)

    model_directory_path.mkdir(parents=True, exist_ok=True)

    stan_model_name = f"model_{model_name.split('/')[1]}"
    cpp_code, _ = pybindstan.compile.compile(program_code, stan_model_name)
    ...
    compiler_output = pybindstan.build_ext.run_build_ext(extensions, build_lib)
